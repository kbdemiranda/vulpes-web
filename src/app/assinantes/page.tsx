  ...
  return (
    <Protected>
      ...
      <section className="theme-transition relative rounded-lg border p-4" style={{ borderColor: "var(--border)", background: "var(--surface)" }}>
        ...
        <ListTable data={data} onView={(id) => router.push(`/assinantes/${id}`)} onEdit={(id) => router.push(`/assinantes/${id}/editar`)} onDelete={async (id) => {
...
type Row = { id: number; nome?: string; email?: string; valor_por_mes?: number };
...
function ListTable({ data, onView, onEdit, onDelete }: { data: any; onView: (id: number) => void; onEdit: (id: number) => void; onDelete: (id: number) => void }) {
      ...
      <table className="w-full text-left text-sm">
        <thead>
          <tr className="border-b" style={{ borderColor: "var(--border)" }}>
            ...
            <th className="px-2 py-2">Valor/mês</th>
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.id} className="border-b last:border-0" style={{ borderColor: "var(--border)" }}>
              ...
              <td className="px-2 py-2">{r.valor_por_mes ?? "-"}</td>
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4">
      <div className="theme-transition w-full max-w-md rounded-lg border p-5" style={{ borderColor: "var(--border)", background: "var(--surface)" }}>
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-semibold">Novo Assinante</h3>
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div>
            ...
            <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full rounded border border-[var(--border)] bg-[var(--input)] p-2" />
          </div>
